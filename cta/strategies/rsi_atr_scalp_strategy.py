...
from .cutility import CBarGenerator, CArrayManager, logger, CoolDownTimer, SignalArrayManager
from datetime import datetime 
import talib
import numpy as np

class RsiAtrScalpStrategy(CtaTemplate):
    """"""
    author = "Xiaohui"
    ...
    variables = ["rsi_value", "rsi_long", "rsi_short","rsi_ma",
                 "k_value","d_value", "ma_trend","stop_loss_limit","atr", "cd", "sl_price", "eg_price", "n_atr_max"]
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        ...
        self.rsi_arry = None
        self.k_am = SignalArrayManager(180)
        self.d_am = SignalArrayManager(180)
        self.am1 = ArrayManager()
        self.am1_rt = CArrayManager()
    ...
    def on_start(self):
        ...
        self.ma_trend = self.mngr_discretion
        self.write_log("策略启动")
    ...
    def on_tick(self, tick: TickData):
        ...
        if not self.am1_rt.inited:
            return
        
        currentBar = self.bg5.bar
        self.am1_rt.update_last_bar(currentBar)
        
        self.k_value, self.d_value = self.am1_rt.stoch(self.k_fast_window,self.k_slow_window, 0, self.k_slow_window, 0)
        self.k_value = round(self.k_value, 3)
        self.d_value = round(self.d_value, 3)
        
        #update real time k, d array
        self.k_am.update_signal(self.k_value)
        self.d_am.update_signal(self.d_value)
        
        open_pos_signal = self.open_position()
        
        #do not open position if atr is low
        if self.atr < currentBar.close_price * self.atr_th/100:
            open_pos_signal = 0
        ...
        if self.pos == 0 and not self.cooldown.initiated:
            if open_pos_signal > 0:
                self.buy(currentBar.close_price + 5 * self.pricetick, self.fixed_size)
                self.pos_lock += self.fixed_size
                self.write_params_log()
            elif open_pos_signal < 0:
                self.short(currentBar.close_price - 5 * self.pricetick, self.fixed_size)
                self.pos_lock -= self.fixed_size
                self.write_params_log()
        
        elif self.pos > 0:
            if self.stop_loss(currentBar) or self.exit_gain(currentBar):
                self.sell(currentBar.close_price - 5 * self.pricetick, abs(self.pos))
                self.pos_lock -= abs(self.pos)
                self.write_params_log()

        elif self.pos < 0:
            if self.stop_loss(currentBar) or self.exit_gain(currentBar):
                self.cover(currentBar.close_price + 5 * self.pricetick, abs(self.pos))
                self.pos_lock += abs(self.pos)
                self.write_params_log()
    ...
    def on_bar(self, bar: BarData):
        ...
        #shift the last bar to hold real time bar
        self.am1_rt.shift_last_bar()
    ...
    def stop_loss(self,bar:BarData):
        ...
        if close_position:
            self.cooldown.start(bar.datetime)
    ...
    def exit_gain(self,bar:BarData):
        ...
        if self.pos > 0:
            if self.rsi_value > self.rsi_short or bar.close_price > self.eg_price:
                exit_position = True
        elif self.pos < 0:
            if self.rsi_value < self.rsi_long or bar.close_price < self.eg_price:
                exit_position = True
        else:
            pass
    ...
    def open_position(self):
        ...
        if not self.k_am.inited or not self.d_am.inited:
            return ls_signal
        
        #long short signal(statistical)
        lb = 120 #lookback 120 ticks
        cl = 0.7 # confidence level 70%
        
        if self.direction > 0 and self.k_am.greater_than(self.d_am.signal, lb, cl):
            ls_signal = 1
            #swith off direction signal to avoid multiple position open with in 1 min
            self.direction = 0
        elif self.direction < 0 and self.k_am.less_than(self.d_am.signal, lb, cl):
            ls_signal = -1
            #swith off direction signal to avoid multiple position open with in 1 min
            self.direction = 0
        
        # #long short signal(simple)
        # if self.direction > 0 and self.k_value > self.d_value:
        #     ls_signal = 1
        #     #swith off direction signal to avoid multiple position open with in 1 min
        #     self.direction = 0
        # elif self.direction < 0 and self.k_value < self.d_value:
        #     ls_signal = -1
        #     #swith off direction signal to avoid multiple position open with in 1 min
        #     self.direction = 0
    ...
    def update_view(self):
        ...
        if self.trend_filter > 0:
            #set up kd watch indicator(with trend filter)
            ...
            #set up kd watch indicator(simple)
    ...
    def write_params_log(self):
        self.write_log("trend:{0};rsi_current:{1};rsi_ma:{2};rsi_long:{3};rsi_short:{4};k:{5};d:{6};pos:{7}"
        .format(self.ma_trend, self.rsi_value, self.rsi_ma, self.rsi_long, self.rsi_short, self.k_value, self.d_value, self.pos))

class RsiAtrScalpQuickStrategy(RsiAtrScalpStrategy):
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        super().__init__(cta_engine, strategy_name, vt_symbol, setting)
...
class RsiAtrScalpSlowStrategy(RsiAtrScalpStrategy):
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        super().__init__(cta_engine, strategy_name, vt_symbol, setting)
