from typing import Callable, Tuple, Type,Union, Dict, List
...
from vnpy_spreadtrading.base import (SpreadData,
                                     LegData,
                                     query_bar_from_datafeed)

d_format_ploty = "%Y-%m-%d %H:%M"
...
class CArrayManager(ArrayManager):
    ...
    def donchian_smooth(
        ...
        Donchian Channel.
        """
        up_h: np.ndarray = talib.MAX(self.high, n)
        up_o: np.ndarray = talib.MAX(self.open, n)
        up_c: np.ndarray = talib.MAX(self.close, n)
        up_l: np.ndarray = talib.MAX(self.low, n)
        
        down_h: np.ndarray = talib.MIN(self.high, n)
        down_o: np.ndarray = talib.MIN(self.open, n)
        down_c: np.ndarray = talib.MIN(self.close, n)
        down_l: np.ndarray = talib.MIN(self.low, n)
        
        up: np.ndarray = np.mean([up_h,up_o,up_c,up_l],axis=0)
        down: np.ndarray = np.mean([down_h, down_o, down_c, down_l],axis= 0)
...
def parse_shapes(row):
    t_string = datetime.strftime(row['datetime'],d_format_ploty)
    return dict(x0=t_string, x1=t_string, y0=0, y1=1, xref='x', yref='paper',line_width=1)
...
def get_next_key(target_key:str,data_dict:dict):
    key_list = list(data_dict)
    res = None
    try:
        res = key_list[key_list.index(target_key) + 1]
    except (ValueError, IndexError):
        res = None
    return res

def update_to_csv(fileName,data_df, keys = ['']):
    if(os.path.exists(fileName)):
        hist_df = pd.read_csv(fileName)
        concat_df=pd.concat([hist_df,data_df])
        concat_df=concat_df.drop_duplicates(subset = keys, keep='last')
    else:
        concat_df=data_df
    concat_df.to_csv(fileName,index=False)

def strip_numbers(input_string) -> str:
...
def lookup_price_tick(ticker:str):
    config = cfg.config_parser()
    pricetick_dict = config['research']['china_fut_pricktick']
    key_ = strip_numbers(ticker).upper()
    price_tick = 0
    unit = 0
    if key_ in pricetick_dict.keys():
        price_tick = float(pricetick_dict[key_][0])
        unit = int(pricetick_dict[key_][1])
    return (price_tick,unit)