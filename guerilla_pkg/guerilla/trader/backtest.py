...
from .tutils import parse_anim,parse_shapes,parse_optm_result_df
...
class BacktestingEngine:
    ...
    def run_group_backtesting(self, group_settings:List[Dict]):
        """A method to do batch run for backtesting.

        Args:
            group_settings (List[Dict]): list of backtest settings
        ...
        if len(group_settings) <1:
            return DataFrame()
        evaluate_func: callable = wrap_evaluate(self, 'annual_return')
        
        self.output(f"开始执行参数组回测")
        self.output(f"参数组样本量：{len(group_settings)}")
        ...
        with ProcessPoolExecutor(
            ...
            results: List[Tuple] = list(executor.map(evaluate_func, group_settings))

            end: int = perf_counter()
            cost: int = int((end - start))
            self.output(f"参数组回测完成，耗时{cost}秒")
            result_dict = [parse_optm_result_df(r) for r in results]
    ...
    def get_trades_df(self):
        ...
        orderid = [int(val.orderid) for (key, val) in self.trades.items()]
    ...
    def get_hist_data_df(self):
        symbol = [bd.symbol for bd in self.history_data]
        exch = [bd.exchange.value for bd in self.history_data]
        datetime_ = [bd.datetime for bd in self.history_data]
        vol = [bd.volume for bd in self.history_data]
        high_price = [bd.high_price for bd in self.history_data]
        low_price = [bd.low_price for bd in self.history_data]
        open_price = [bd.open_price for bd in self.history_data]
        close_price = [bd.close_price for bd in self.history_data]
        hist_df = DataFrame()
        hist_df['datetime'] = datetime_
        hist_df['symbol']=symbol
        hist_df['exch']=exch
        hist_df['open_price']=open_price
        hist_df['close_price']=close_price
        hist_df['high_price']=high_price
        hist_df['low_price']=low_price
        hist_df['volume']=vol
        return hist_df
            
    def show_trades(self, start_date:str = '', end_date:str = ''):
        d_format_short = "%Y-%m-%d"
        
        if not start_date or not end_date:
            print('input date issue, please check')
            return
        
        s_date = datetime.strptime(start_date,d_format_short)
        e_date = datetime.strptime(end_date,d_format_short) + timedelta(days=1)
        china_tz = timezone('Asia/Shanghai')
        e_date = e_date.replace(tzinfo=china_tz)
        s_date = s_date.replace(tzinfo=china_tz)
        
        hist_df = self.get_hist_data_df()
        trades_df = self.get_trades_df()
        hist_visual_df = hist_df.loc[(hist_df['datetime']>=s_date) & (hist_df['datetime']<=e_date)]
        trades_visual_df = trades_df.loc[(trades_df['datetime']>=s_date) & (trades_df['datetime']<=e_date)]
        
        if len(hist_visual_df) > 0 and len(trades_visual_df) > 0:
            s = trades_visual_df.apply(parse_shapes, axis=1)
            a = trades_visual_df.apply(parse_anim, axis = 1)
            fig = go.Figure(data=[go.Candlestick(x=hist_visual_df['datetime'],
                open=hist_visual_df['open_price'],
                high=hist_visual_df['high_price'],
                low=hist_visual_df['low_price'],
                close=hist_visual_df['close_price'])])
            fig.update_layout(
                title='Trades during period ' + datetime.strftime(s_date,d_format_short) + ' to ' + datetime.strftime(e_date,d_format_short),
                yaxis_title=self.vt_symbol,
                shapes = s.tolist(),
                annotations=a.tolist())
            fig.show()
        else:
            print("cannot plot: {0} historical data found and {1} trades found.".format(len(hist_visual_df),len(trades_visual_df)))
            return
